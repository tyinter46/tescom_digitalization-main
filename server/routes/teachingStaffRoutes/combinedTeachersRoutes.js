...
Router.get('/', async (req, res)=>{
    try {
        //console.log(pool)
        const teachers = await pool.query("SELECT * from combined_school_teachers")
...
Router.get('/:id', async (req, res)=>{
     try {
        ...
        const teacher = await pool.query("SELECT * FROM combined_school_teachers WHERE id = $1", [id])  
...
//Delete One combined school Teacher and add to table of retired teachers 
 Router.delete('/:id', async (req, res)=>{
    try {
const {id} = req.params
const idExist = await pool.query("SELECT EXISTS (select * from combined_school_teachers WHERE id = $1)", [id]); 
if (!idExist.rows[0].exists) throw new Error ("staff not available")  
const deleteOneSeniorSchoolTeacher = await pool.query("DELETE FROM combined_school_teachers WHERE id = $1 RETURNING *", [id])
